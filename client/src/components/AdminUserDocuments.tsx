...
import { Users, FileText, Loader2, Sparkles } from 'lucide-react';
...
export const AdminUserDocuments: React.FC = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        if (res.data.success) {
          // Filter hanya user biasa (klien) atau semua user non-admin utama
        ...
        console.error('Error fetching users for documents:', err);
      } finally {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6 pt-4">
        <div className="p-4 rounded-2xl bg-white border border-slate-200 shadow-sm flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-sm font-bold text-slate-900">Kelola Dokumen & Brosur Klien</h2>
              <p className="text-xs text-slate-500">
                Pilih akun klien untuk mengunggah, meninjau, atau menghapus brosur dan berkas AI milik mereka
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-2">
            ...
            {loadingUsers ? (
              ...
              <select
                ...
                className="text-xs font-semibold rounded-xl border border-slate-300 bg-slate-50 px-3 py-2 text-slate-900 focus:border-indigo-600 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 shadow-xs"
              >
                {users.map((u) => (
      ...
      {/* Embedded Document Knowledge Component */}
      {selectedUser ? (
        <DocumentKnowledge
        ...
        <div className="text-center py-20 text-slate-400 text-xs">
          Silakan pilih akun klien di atas untuk mengelola dokumen.
        </div>
      )}
    </div>
  );
